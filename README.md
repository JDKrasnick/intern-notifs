...
4. Store the production runtime configuration as an encrypted SSM SecureString named `/intern-notifs/runtime-config` (the Lambda reads only its name, never the values from source control):
   ...
   The defaults make a compact phone notification: its title is `Short role — Company` (for example, `SWE — Acme` or `ML — Acme`). Its description has the full role, location, season, pay when known, inferred focus tags, posting date when a source supplies one, and the apply link. Each user can change `ntfyTitleTemplate` and `ntfyDescriptionTemplate` without a redeploy. Valid placeholders are `{title}`, `{shortTitle}`, `{company}`, `{location}`, `{season}`, `{compensation}`, `{compensationDetail}`, `{focus}`, `{posted}`, `{postedDetail}`, `{source}`, and `{url}`. `focus` is inferred from the role/source metadata (for example, `SWE`, `AI/ML`, or `Quant`); it is not an invented qualification list. For example, `"{company}: {title}"` keeps the full title; `"{location}\n{compensation}\n{url}"` keeps the description extra short. `ntfyRoleAbbreviations` can add or override the default role aliases (`software engineering` → `SWE`, `machine learning` → `ML`, `artificial intelligence` → `AI`, `data science` → `DS`, `product management` → `PM`, `quantitative` → `Quant`). Tapping an individual alert opens its application URL.
