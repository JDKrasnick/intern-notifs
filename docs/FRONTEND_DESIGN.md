...
Place Gmail application detection in **App & account**. Before consent, explain in plain language that after an Apply click InternNotifs checks for that role after 5 minutes, 10 minutes, 30 minutes, and 24 hours, reading only sender, subject, date, and labels. State that bodies and attachments are never read and Gmail data is not used for AI or model training.
