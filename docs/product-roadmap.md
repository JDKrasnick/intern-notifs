...
- [ ] Complete Ashby production shadow verification and per-board promotion: the scheduler, queues, persistence, alarms, shared dashboard fields, quarantine/recovery controls, and enforced promotion-evidence gate are deployed; the original 21 boards began link-checked evidence collection on 2026-08-09 and 14 owner-approved boards entered healthy shadow monitoring later that day, so each board still needs three clean snapshots spanning at least 24 hours and owner approval after its latest snapshot before promotion.
