...
- [ ] Complete issue #134's employer metadata enrichment rollout.
  ...
  - [x] Implement v8 housing support/cost evidence and detail display, broader metadata correctness checks, additional pay formats, and explicitly reviewed omissions with atomic evidence/review guards. Deployment and historical publication require the remaining rollout gates below.
  - [ ] Finish the independently reviewed cohort, verify all provider canaries and native rendering. See [the delivery plan](metadata-coverage-plan.md).
