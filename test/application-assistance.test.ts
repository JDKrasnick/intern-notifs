...
describe('reviewed application assistance registry', () => {
  it('supports the reviewed Greenhouse, direct Lever, and published Ashby form routes', () => {
    ...
    expect(assistanceAvailability({ applyUrl: 'https://jobs.ashbyhq.com/etched/123e4567-e89b-12d3-a456-426614174000/application' })).toMatchObject({
      eligibility: 'headed-supported', reasonCode: 'ashby-headed-pilot', primaryAction: 'assist-in-safari',
    });
  });

  it('fails closed for incomplete Lever URLs, unreviewed Ashby boards, prohibited destinations, arbitrary sites, and partner submission routes', () => {
    ...
    expect(assistanceAvailability({ applyUrl: 'https://jobs.ashbyhq.com/not-reviewed/123e4567-e89b-12d3-a456-426614174000/application' })).toMatchObject({ eligibility: 'manual-only', reasonCode: 'destination-not-reviewed' });
