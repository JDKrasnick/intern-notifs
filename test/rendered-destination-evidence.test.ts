...
describe('rendered destination frame evidence', () => {
  ...
  it('does not close the requested posting from related-role copy in the same frame', () => {
    const role = listing();
    const evidence = combineRenderedFrameEvidence({ role: role.title, expectedPostingId: '1234567', frames: [frame({
      ...
      structuredJobText: JSON.stringify({ '@type': 'JobPosting', identifier: '1234567', title: role.title,
        description: 'Build reliable systems.' }),
      jobPostingCount: 1,
      applicationFormPresent: true,
    })] })!;
    expect(evidence).toMatchObject({ closureState: 'open', postingIdPresent: true, jobPostingCount: 1 });
    expect(classifyDestination({ listing: role, reachability: 'live', evidence, browserVisible: true,
      inspectedAt: '2026-08-28T00:00:00Z' }).classification).toBe('application-form');
  });
  ...
  it('still honors explicit closure in the selected structured posting', () => {
    ...
    const evidence = combineRenderedFrameEvidence({ role: role.title, expectedPostingId: '1234567', frames: [frame({
      title: role.title,
      visibleText: `${role.title} Job ID 1234567`,
      structuredJobText: JSON.stringify({ '@type': 'JobPosting', identifier: '1234567', title: role.title,
        description: 'This job has expired.' }),
      jobPostingCount: 1,
    })] })!;
    expect(evidence).toMatchObject({ closureState: 'gone', closureSignal: 'explicit-language' });
  });
