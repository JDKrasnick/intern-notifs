...
describe('Lever queue dispatch', () => {
  ...
  it('uses elapsed quiet cadence on the deployed hourly schedule and catches up after a missed run', () => {
    const lastSuccessAt = '2026-07-30T00:00:00.000Z';
    const checkpoint = { sourceId: `shadow-${shadowSource.id}`, successfulFetches: 1, lastRowCount: 0, lastSuccessAt };
    const hourlyRuns = Array.from({ length: 7 }, (_, hour) => new Date(Date.parse(lastSuccessAt) + hour * LEVER_POLL_INTERVAL_MS));
    ...
    expect(isLeverSourceDue(shadowSource, checkpoint, new Date(Date.parse(lastSuccessAt) + 10 * LEVER_POLL_INTERVAL_MS))).toBe(true);
