...
describe('AWS runtime commands', () => {
  it('uses the same quiet-baseline poll behavior as the CLI', async () => {
    ...
    const result = await runRuntimeCommand('poll', { store, config: { smsDestination: '+12025550123', sesFrom: 'a@example.com', sesTo: 'b@example.com' }, sources: [adapter], smsPublisher: { publish: async (message) => { messages.push(message); } } });
    expect(result).toMatchObject({ poll: { baselineSources: ['fixture'], newJobs: [] }, sms: { sent: 0, failed: 0 } }); expect(messages).toEqual([]);
  });
  it('does not send an empty digest', async () => {
    const store = new MemoryInternshipStore(); let sent = false;
    expect(await runRuntimeCommand('digest', { store, config: { smsDestination: '+12025550123', sesFrom: 'a@example.com', sesTo: 'b@example.com' }, emailSender: { send: async () => { sent = true; } } })).toEqual({ digested: 0 }); expect(sent).toBe(false);
  });
});
