...
export interface InternshipStore {
  ...
  listOpen?(cursor?: string, limit?: number): Promise<{ jobs: Internship[]; cursor?: string }>;
}
...
export class MemoryInternshipStore implements InternshipStore {
  ...
  async getJob(jobId: string) { const job = this.jobs.get(jobId); return job && structuredClone(job); }
  async listOpen(cursor?: string, limit = 25) { const jobs = [...this.jobs.values()].filter((job) => job.open && isTechnicalJob(job)).sort((a, b) => b.firstSeenAt.localeCompare(a.firstSeenAt)); const offset = cursor ? Number(cursor) : 0; const page = jobs.slice(offset, offset + limit).map((job) => structuredClone(job)); return { jobs: page, cursor: offset + page.length < jobs.length ? String(offset + page.length) : undefined }; }
}

type JobItem = { pk: string; sk: 'META'; urlPk: string; fingerprintPk: string; smsPk?: string; digestPk?: string; openPk?: string; openSk?: string; job: Internship };
...
export class DynamoInternshipStore implements InternshipStore {
  ...
  async getJob(jobId: string): Promise<Internship | undefined> { const result = await this.client.send(new GetCommand({ TableName: this.tableName, Key: { pk: `JOB#${jobId}`, sk: 'META' } })); return result.Item?.job as Internship | undefined; }
  async listOpen(cursor?: string, limit = 25): Promise<{ jobs: Internship[]; cursor?: string }> {
    const result = await this.client.send(new QueryCommand({ TableName: this.tableName, IndexName: 'openJobsIndex', KeyConditionExpression: 'openPk = :open', ExpressionAttributeValues: { ':open': 'OPEN' }, ScanIndexForward: false, Limit: limit, ...(cursor ? { ExclusiveStartKey: JSON.parse(Buffer.from(cursor, 'base64url').toString('utf8')) } : {}) }));
    return { jobs: (result.Items ?? []).map((item) => item.job as Internship), ...(result.LastEvaluatedKey ? { cursor: Buffer.from(JSON.stringify(result.LastEvaluatedKey)).toString('base64url') } : {}) };
  }
}
...
export class MemoryUserStore implements UserStore {
  ...
  async deleteUser(userId: string) { const docs = await this.listDocuments(userId); for (const map of [this.preferences, this.profiles]) map.delete(userId); for (const [key] of this.devices) if (key.startsWith(`${userId}#`)) this.devices.delete(key); for (const [key] of this.applications) if (key.startsWith(`${userId}#`)) this.applications.delete(key); for (const [key] of this.documents) if (key.startsWith(`${userId}#`)) this.documents.delete(key); return docs; }
}
...
export class DynamoUserStore implements UserStore {
  ...
  async activeDevices() { const result = await this.client.send(new QueryCommand({ TableName: this.tableName, IndexName: 'activeDevicesIndex', KeyConditionExpression: 'activePk = :active', ExpressionAttributeValues: { ':active': 'ACTIVE' } })); return (result.Items ?? []).map((item) => item.value as DeviceToken); }
  ...
  async listApplications(userId: string) { const response = await this.client.send(new QueryCommand({ TableName: this.tableName, KeyConditionExpression: 'pk = :pk AND begins_with(sk, :prefix)', ExpressionAttributeValues: { ':pk': `USER#${userId}`, ':prefix': 'APPLICATION#' }, ScanIndexForward: false })); return (response.Items ?? []).map((item) => item.value as ApplicationRecord); }
  ...
  async listDocuments(userId: string) { const response = await this.client.send(new QueryCommand({ TableName: this.tableName, KeyConditionExpression: 'pk = :pk AND begins_with(sk, :prefix)', ExpressionAttributeValues: { ':pk': `USER#${userId}`, ':prefix': 'DOCUMENT#' } })); return (response.Items ?? []).map((item) => item.value as UserDocument); } putDocument(value: UserDocument) { return this.put(value.userId, `DOCUMENT#${value.documentId}`, 'document', value); } async deleteDocument(userId: string, documentId: string) { await this.client.send(new DeleteCommand({ TableName: this.tableName, Key: { pk: `USER#${userId}`, sk: `DOCUMENT#${documentId}` } })); }
  ...
  async pendingReceipts() { const result = await this.client.send(new QueryCommand({ TableName: this.tableName, IndexName: 'pendingReceiptsIndex', KeyConditionExpression: 'receiptPk = :pending', ExpressionAttributeValues: { ':pending': 'PENDING' } })); return (result.Items ?? []).map((item) => item.value as DeliveryReceipt); }
  async deleteUser(userId: string) { const documents = await this.listDocuments(userId); const response = await this.client.send(new QueryCommand({ TableName: this.tableName, KeyConditionExpression: 'pk = :pk', ExpressionAttributeValues: { ':pk': `USER#${userId}` } })); await Promise.all((response.Items ?? []).map((item) => this.client.send(new DeleteCommand({ TableName: this.tableName, Key: { pk: item.pk, sk: item.sk } })))); return documents; }
}
