...
export async function runLeverBoard(
  ...
  const poll = await new Poller([adapter], dependencies.store, undefined, undefined, validate, false, dependencies.enqueueDestinationVerification).poll({ runId: message.runId });
